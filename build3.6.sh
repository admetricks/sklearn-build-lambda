...
strip_virtualenv () {
    ...
    find $VIRTUAL_ENV/lib64/python3.6/site-packages/ -name "*.so" | grep -v /PIL/ | xargs strip
    echo "venv stripped size $(du -sh $VIRTUAL_ENV | cut -f1)"

    pushd $VIRTUAL_ENV/lib/python3.6/site-packages/ && zip -r -9 -q /outputs/venv.zip joblib* ; popd
...
main () {
    /usr/bin/python3.6 -m venv --copies /sklearn_build
